/// Servicio HTTP para medicación del expediente (Módulo 3).
import 'dart:convert';
...
class MedicationService {
  ...
  Future<List<Map<String, dynamic>>> list(int petId) async {
    final res = await http.get(
      Uri.parse('${ApiConfig.baseUrl}/pets/$petId/medications'),
    ...
    throw Exception('No se pudo cargar la medicación (${res.statusCode})');
  }
  ...
  Future<void> remove(int petId, int id) async {
    final res = await http.delete(
      Uri.parse('${ApiConfig.baseUrl}/pets/$petId/medications/$id'),
      headers: await _headers(),
    );
    if (res.statusCode != 200 && res.statusCode != 201) {
