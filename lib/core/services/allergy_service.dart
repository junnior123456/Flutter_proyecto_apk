/// Servicio HTTP para alergias del expediente (Módulo 3).
import 'dart:convert';
...
class AllergyService {
  ...
  Future<List<Map<String, dynamic>>> list(int petId) async {
    final res = await http.get(
      Uri.parse('${ApiConfig.baseUrl}/pets/$petId/allergies'),
    ...
    throw Exception('No se pudieron cargar las alergias (${res.statusCode})');
  }
  ...
  Future<void> remove(int petId, int id) async {
    final res = await http.delete(
      Uri.parse('${ApiConfig.baseUrl}/pets/$petId/allergies/$id'),
      headers: await _headers(),
    );
    if (res.statusCode != 200 && res.statusCode != 201) {
